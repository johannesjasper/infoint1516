...
INSERT INTO target.school(name, location_id)
  ...
  FROM baseballarchiv.schools, target.location 
...
INSERT INTO target.person(first_name, last_name, nick_name, birth_date, birth_location, death_date, death_location, weight, height)
  SELECT 
    ...
    weight,
    height
  FROM baseballarchiv.master, target.location birth_location, target.location death_location
  WHERE 
...
INSERT INTO target.person_school(person_id, school_id)
  ...
  FROM 
    target.person, target.school, target.location school_location,
...
INSERT INTO target.award(person_id, name)
  SELECT DISTINCT person.person_id person_id, ('Hall of fame ' || halloffame.yearid) "name"
  FROM baseballarchiv.master, target.person, baseballarchiv.halloffame
  WHERE
    person.first_name = master.namegiven AND person.last_name = master.namelast AND person.nick_name = master.namefirst AND
